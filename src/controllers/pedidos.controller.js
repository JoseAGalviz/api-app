...
async function insertarPedidoCotizC(pool, valores) {
  try {
    ...
    console.log('Inserción en cotiz_c exitosa.');
  } catch (err) {
...
async function registrarPista(pool, fact_num, codigo_pedido, fecha_caracas) {
  try {
    ...
    console.log('Inserción en pistas exitosa.');
  } catch (err) {
...
//       .input('cod_pedido', sql.VarChar, String(cod_pedido))
//       .query(`
//         SELECT fact_num 
//         FROM cotiz_c 
...
//       });
//     }
//     // --- FIN VERIFICACIÓN DE DUPLICADO ---

//     // --- LÓGICA DE APOYO (Para evitar el error de "is not defined") ---
...
//     // --- PRE-CÁLCULO DE TOTALES POR PROVEEDOR ---
...
//     const descuento_porc = (porc_gdesc !== undefined && porc_gdesc !== null) ? Number(porc_gdesc) : (desc_glob_db || 0);
...
//         const porc_desc_cadena = await calcularCadenaTripleInterna(
//             pool, cod_cliente, co_art_limpio, co_cat, co_prov, totalAcumuladoProv
//         );
...
//           rowguid: uuidv4()
//         };
//         await insertarPedidoCotizC(pool, valoresCotizC);
//       } catch (err) {
...
//           if (cant <= 0) return;
//           const v = {
//             fact_num, reng_num, co_art: dr.co_art_limpio, co_alma: alma, total_art: cant, pendiente: cant,
//             uni_venta: dr.uni_venta_db, prec_vta: (dr.precio_cliente * tasa), porc_desc: dr.porc_desc,
//             tipo_imp: dr.tipo_imp_db, reng_neto: ((cant * dr.precio_cliente) * tasa),
//             prec_vta2: dr.precio_cliente, des_art: dr.des_art, rowguid: uuidv4(), fec_lote: fecha_caracas_datetime
//           };
//           const cols = Object.keys(v).join(', ');
//           const params = Object.keys(v).map((_, i) => `@p${i}`).join(', ');
//           const reqReng = pool.request();
...
//     // --- VALIDACIÓN co_ven 00027: se evalúa una sola vez por pedido ---
...
//       // --- SET para acumular co_prov con escalas aplicadas en este batch ---
...
//         // --- SI TIENE ALGÚN DESCUENTO DE ESCALA, AGREGAR co_prov AL SET ---
//         const tieneEscala = porc_desc_cadena
//             .split('+')
//             .some(v => Number(v) > 0);
//         if (tieneEscala && co_prov) {
//             provConEscala.add(co_prov);
//         }
...
//       // --- CAMPO5: co_prov de artículos con escalas aplicadas separados por coma ---
//       const campo5 = '';
...
//           tasag: 16, 
//           campo3: campo3 || '',
//           campo5,                   // ← co_prov con escalas aplicadas
//           co_us_in: 'API',
//           fe_us_in: fecha_caracas_datetime, 
//           co_sucu: '01', 
//           rowguid: uuidv4()
...
// ─── FUERA del handler (scope global del módulo) ───────────────────────────
...
async function registrarPistaPedido(pool, fact_num, codigo_pedido, fecha_caracas) {
  try {
    ...
    console.log(`[Pistas]: Registro de auditoría completado para Pedido Profit #${fact_num}`);
  } catch (err) {
...
export const insertarPedido = async (req, res) => {
  console.log('Iniciando inserción de Pedido Psico:', req.body.cod_pedido);
  const resultado = {
    pedido_montado: false,
    fact_nums: [],
    errores: [],
